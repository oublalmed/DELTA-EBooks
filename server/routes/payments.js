  ...
  (async () => {
    try {
      ...
      db.prepare(`
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
      `).run(req.user.id, bookId, order.id, 'ORDER_CREATED', book.price, book.currency, order.status, req.ip);
      ...
      db.prepare(`
        ...
        VALUES (?, ?, ?, ?, ?, ?)
      `).run(req.user.id, req.body.bookId || '', 'ORDER_CREATION_FAILED', 'error', err.message, req.ip);
...
router.post('/capture-order', requireAuth, (req, res) => {
  (async () => {
    try {
      ...
      if (captureData.status !== 'COMPLETED') {
        db.prepare(`
          ...
          VALUES (?, ?, ?, ?, ?, ?, ?)
        `).run(req.user.id, bookId, orderId, 'CAPTURE_INCOMPLETE', captureData.status, JSON.stringify(captureData), req.ip);
      ...
      if (capturedAmount < book.price) {
        db.prepare(`
          ...
          VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        `).run(req.user.id, bookId, orderId, 'AMOUNT_MISMATCH', capturedAmount, 'error',
      ...
      db.prepare(`
        ...
        VALUES (?, ?, ?, ?, ?, ?)
      `).run(req.user.id, bookId, orderId, capturedAmount, book.currency, 'completed');

      // Log successful capture
      db.prepare(`
        INSERT INTO payment_logs (user_id, book_id, paypal_order_id, event_type, amount, currency, status, raw_data, ip_address)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
      `).run(req.user.id, bookId, orderId, 'PAYMENT_CAPTURED', capturedAmount, book.currency, 'completed', JSON.stringify(captureData), req.ip);
      ...
      db.prepare(`
        INSERT INTO payment_logs (user_id, book_id, paypal_order_id, event_type, status, raw_data, ip_address)
        VALUES (?, ?, ?, ?, ?, ?, ?)
      `).run(req.user.id, req.body.bookId || '', req.body.orderId || '', 'CAPTURE_FAILED', 'error', err.message, req.ip);
